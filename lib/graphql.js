...
function getPaths(query) {
  ...
  if (
    ...
    query.selectionSet.selections.length > 0
  ) {
    ...
    query.kind === "InlineFragment" &&
    query.typeCondition &&
    query.typeCondition.kind === "NamedType"
  ) {
...
function make_node(field, origin, definitions) {
  ...
  return {
    ...
    children: [],
    scopes: get_scopes_from_field(field).concat(
      get_scopes_from_type(definitions[type_name])
    ),
    origin: origin || false,
    type: type_name,
  };
...
function schemaToGraph(parsedSchema) {
  ...
  if (Object.prototype.hasOwnProperty.call(definitions, "Query")) {
  ...
  if (Object.prototype.hasOwnProperty.call(definitions, "Mutation")) {
  ...
  while (stack.length > 0) {
    ...
    if (last_node.scopes.length > 0) {
      last_node.scopes.forEach(function (scope) {
        ...
        paths[scope.name] = paths[scope.name].concat(
          make_entitlements(path, scope)
        );
      });
    }
